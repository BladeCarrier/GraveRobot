...
class system:
    ...
    def fullLoop(self,samples=[]):
        ...
        if self.writeLog:
            ...
            self.logWriter.writeLine('trees population onafterstep', len(self.treesPool))
    ...
    def addChromo(self,newCh):
        '''adds an existing & ready for classification chromo to this system'''
        self.treesPool.add(newCh)
        for sample in newCh.samples:
            if sample not in self.samplesPool:
                self.samplesPool[sample] = set()
            self.samplesPool[sample].add(newCh)
        self.currentNodesCount+= newCh.numNodes
        self.currentSamplesCount += len(newCh.samples)
        self.currentAttrsCount += newCh.lenStatements()
        return newCh
    def getBestChromos(self,count = 1):
        '''returns copies of best chromos used for sharing'''
        pool = list(self.treesPool)
        pool.sort(key = lambda x: x.fitness,reverse = True)
        return [copy.deepcopy(ch) for ch in pool[:count]]
    ...
    def classify(self,sample,threshold = 0,maxCount = -1):  
        ...
        for ch in pool:
            ...
            cresult = ch.classify(sample)
            
            r = tuple([cresult[state] for state in order])
            
            if r not in options:
                options[r] = 0#add new option
        ...
        for option in options:
            ...
            if val>maxVal:
                maxVal = val
                maxOpt = option
        return {order[i]:maxOpt[i] for i in range(len(maxOpt))}
