...
class chromo:
    ...
    def prune(self,node):
        ...
        if state not in self.stateUseDict:#debug row
            print state.toString
            print 'me:' ,self.fitness
        self.stateUseDict[state].remove(node) #once caused error: state not in stateUseDict
    ...
    def logState(self,writer):
        ...
        for state in self.stateUseDict:
            writer.writeLine(len(self.stateUseDict[state]),':',state.toString())
