...
class statement:
    ...
    def getComplexity(self):
        if self.operation == op_takeConstant or self.operation == op_takeValue:
            return 1
        return sum(i.getComplexity() for i in self.args)+1
    def toString(self):
        arr = []
        for i in self.args:
...
def get_sum(argStatements):
    ...
    newsttmnt.args = set()
    argStatements = list(argStatements)
    while len(argStatements)!=0: #sort by hash
        state = argStatements.pop()
        if state.operation == op_sum:
            argStatements+=state.args
        else:
            newsttmnt.args.add(state)
    return newsttmnt
...
def op_takeValue(args,sample,boolean = False):
    '''operation: take value with args[0] as name'''
    #if it throws no attr exception here, it means that some of samples have some of attrs <available through statements> not set
    if not hasattr(sample,args[0]):
        ans = 0
    else:
        ans = getattr(sample,args[0])
    if boolean:
        ans = bool(round(ans))
    return ans #<- arg is a string. It's a statement elsewhere
