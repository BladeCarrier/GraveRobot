...
def proportionalrandomNoRep(items, fpriority, count):
    ...
    while(len(r) != count):
        for el in set(proportionalrandom(items,fpriority,count-len(r))):
            ...
            r.append(el)
    return r
def proportionalrandomRounds(items, fpriority, count):
    """same as proportional random but recalculates priority every time it choses one item"""
    items = copy.copy(items)
    assert(len(items)!=0 or count == 0)
